...
import { fetchBooks, changeOffset } from "./booksListSlice";
import { PAGINATION_MARGIN } from "../../constants";
...
const BooksList = ({ items, onBookItemClick }) => {
  ...
  const onFetchClicked = useCallback(() => {
    ...
    dispatch(changeOffset(PAGINATION_MARGIN));
  ...
  const renderNoBooksMessage = () => {
    if (booksFound === false && booksLoadingStatus === "idle") {
      return (
  ...
  const renderLoadMoreButton = () => {
    if (items.length > 0) {
      ...
      } else if (booksLoadingStatus === "idle") {
        return (
